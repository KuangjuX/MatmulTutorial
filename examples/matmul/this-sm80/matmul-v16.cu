...
__global__ void matmul(TA *A, TB *B, TC *C, int M, int N, int K) {
    ...
    auto tCsA = local_partition(sA, threadC, threadIdx.x,
                                Step<_1, X>{});  // (THR_M,BLK_K)
    // Partition sB (N,K) by the cols of tC
    auto tCsB = local_partition(sB, threadC, threadIdx.x,
                                Step<X, _1>{});  // (THR_N,BLK_K)
    // Partition gC (M,N) by the tile of tC
    auto tCgC = local_partition(gC, threadC, threadIdx.x,
...
void test_gemm(int m, int n, int k) {
    ...
    std::cout << "Using shared memory = " << (double)smem_size / 1e3
              << " KB.\n";
    if (smem_size >= (48 << 10)) {
        cudaFuncSetAttribute(matmul<TA, TB, TC>,
    ...
    auto host_matrix_to_const_column_major_cute_tensor =
        [](const auto &X, int num_rows, int num_cols, int LDX) {
            ...
            return cute::make_tensor(X.data(),
