...
/// RUN: nvcc -arch=sm_89 -std=c++17  -DDEBUG -Xcompiler -fopenmp -I
/// /home/zhengsz/share/MatmulTutorial/3rdparty/cutlass/include matmul-v17.cu
/// main.cu -o test && ./test stages 4

/// RUN: nvcc -arch=sm_80 -Xcompiler -fopenmp matmul-v17.cu main.cu -o test -I
/// ../../../3rdparty/cutlass/include/ && ./test stages 4
/// TFLOPS: 9296.08
...
__global__ void matmul(half *A, half *B, half *C, int M, int N, int K,
    ...
    using SmemLayoutA = decltype(tile_to_shape(
        SmemLayoutAtomA{}, Shape<Int<BLOCKM>, Int<BLOCKK>, Int<Stages>>{}));
    ...
    using SmemLayoutB = decltype(tile_to_shape(
        SmemLayoutAtomB{}, Shape<Int<BLOCKN>, Int<BLOCKK>, Int<Stages>>{}));
    ...
    using TiledMma =
        TiledMMA<MMA_Atom<SM80_16x8x16_F32F16F16F32_TN>,
                 ...
                 Layout<Shape<_1, _2, _1>>   // 1x2x1 or 1x2x2 value group for
    ...
    auto gA = make_tensor(make_gmem_ptr(A), make_shape(M, K),
                          make_stride(K, Int<1>{}));
    auto gB = make_tensor(make_gmem_ptr(B), make_shape(N, K),
                          make_stride(K, Int<1>{}));
    ...
    auto gAblk =
        local_tile(gA, blk_shape, blk_coord, Step<_1, cute::Underscore, _1>{});
    auto gBblk =
        local_tile(gB, blk_shape, blk_coord, Step<cute::Underscore, _1, _1>{});
    auto gCblk =
        local_tile(gC, blk_shape, blk_coord, Step<_1, _1, cute::Underscore>{});
#ifdef DEBUG
    /// Print debug
    if (thread0()) {
        print("SmemLayoutA: ");
    ...
    SharedStorage &smem_storage =
        *reinterpret_cast<SharedStorage *>(raw_smem_ptr);
    auto sAblk =
        make_tensor(make_smem_ptr(smem_storage.smem_a.data()), SmemLayoutA{});
    auto sBblk =
        make_tensor(make_smem_ptr(smem_storage.smem_b.data()), SmemLayoutB{});
    ...
    if (thread0()) {
        ...
        static_assert(
            extent<SM80_CP_ASYNC_CACHEALWAYS<cute::uint128_t>::SRegisters>::
                value == 1);
        print(size(recast<uint128_t>(gAthread(_, _, _, 0))));
        print("\n");
        ...
        print("\n");
        print(
            "(recast<uint128_t>(group_modes<1,3>(gAthread(_,_,_,0))(_,0))): ");
        print(
            (recast<uint128_t>(group_modes<1, 3>(gAthread(_, _, _, 0))(_, 0))));
        print("\n");
        ...
        print("\n");
        print(
            "(recast<uint128_t>(group_modes<1,4>(flatten(gAthread(_,_,_,0)))(_,"
            "0))): ");
        print((recast<uint128_t>(
            group_modes<1, 4>(flatten(gAthread(_, _, _, 0)))(_, 0))));
        print("\n");
...
#pragma unroll 4
    for (int k = 0; k < Stages - 1; ++k) {
        copy(gmem_tiled_copy_A, gAthread(_, _, _, kouter),
             sAthread(_, _, _, k));
        copy(gmem_tiled_copy_B, gBthread(_, _, _, kouter),
             sAthread(_, _, _, k));
        cp_async_fence();
        --kmax;
        if (kmax > 0) {
