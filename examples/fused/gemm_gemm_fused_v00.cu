...
__device__ void loadSmemB(half *smem, half *B, int N, int K, int ko) {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        // layout: [row_out, col_out, row_in, col_in] = [8, 2, 16, 16]
        smem[row / 16 * (2 * 16 * 16) + col / 16 * (16 * 16) + row % 16 * 16 +
             col % 16] = B[(bx * 128 + row) * K + ko * KI + col];
    }
}
...
__device__ void loadSmemC(float *smem, half *C, int M, int N) {
    ...
    for (int i = 0; i < 128; ++i) {
        ...
        smem[row / 16 * (8 * 16 * 16) + col / 16 * (16 * 16) + row % 16 * 16 +
             col % 16] = (float)(C[(by * 128 + row) * N + bx * 128 + col]);
    }
}
...
__device__ void storeSmemC(half *C, float *smem, int M, int N) {
    ...
    for (int i = 0; i < 128; ++i) {
        ...
        (C[(by * 128 + row) * N + bx * 128 + col]) =
            (half)smem[row / 16 * (8 * 16 * 16) + col / 16 * (16 * 16) +
                       row % 16 * 16 + col % 16];
    }
}
...
__device__ void storeAccum(
    float *ptr, nvcuda::wmma::fragment<nvcuda::wmma::accumulator, wmmaM, wmmaN,
                                       wmmaK, float> *frag) {
    // store 64x64
    int ty = threadIdx.y;
    int tz = threadIdx.z;
...
__global__ void matmul_matmul_fused(half *A, half *B, half *C, half *D, int M,
                                    int N, int K, int Q) {}